...
- [x] В приложении доступен эндпоинт `/metrics` на выделенном порте (порт основного приложения != порт `/metrics`)
- [x] compose-файл содержит настроенные сервисы `prom/prometheus` и `grafana/grafana-oss`
