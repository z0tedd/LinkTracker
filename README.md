...
- [ ] Тест на ретраи (см. Stateful Behaviour у Wiremock)
  - [ ] в т.ч. проверить, что ретраи инициируются только для определенных кодов ошибок
- [ ] Интеграционный тест на rate limiting: поднять приложение, задудосить эндпойнт и получить ошибку 429
- [ ] Тест на circuit breaker: ошибка должна возвращаться до того, как превышен таймаут (в Wiremock можно настроить задержку перед ответом)
