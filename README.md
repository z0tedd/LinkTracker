...
- [x] Некорректные (не парсится и т.п.) сообщения отправляются в отдельную очередь dead letter queue
...
- [x] Валидное сообщение вычитывается и обрабатывается
- [x] Невалидные сообщения перенаправляются в DLQ: ошибки парсинга, валидации
- [x] Кэш подключен: он перехватывает обращения на чтение и инвалидируется при изменении данных
- [x] Валидный JSON корректно мапится в DTO
